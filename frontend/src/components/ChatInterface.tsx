...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ className = "" }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const botReply = await getBotResponse(input); // Wait for API response
  ...
  const API_URL = 'https://d80e-35-192-204-233.ngrok-free.app/chat';
  ...
  return (
    <div className={`flex flex-col h-full chat-container rounded-none overflow-hidden ${className}`}>
      ...
      <div className="p-4 chat-input-container">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={!input.trim()}
            className={`p-3 rounded-full chat-button ${input.trim()
